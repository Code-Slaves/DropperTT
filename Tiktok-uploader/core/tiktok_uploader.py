...
def uploadVideo(session_id, video_content: bytes, title, tags, schedule_time=0, verbose=True, proxy: str = None, auth: dict = None) -> bool:
	...
	session.proxies = None if not proxy else {'http': f'http://{proxy}',}
	session.auth = None if not auth else HTTPProxyAuth(auth["login"], auth["password"])
	...
	session.cookies.set("tt-target-idc", "useast1a", domain=".tiktok.com")
	...
	session.cookies.set('tt-target-idc', None)
